...
//	}
	...
	var TRIPLE_SUBJECT = '(<{2}|<{4}|<{6})(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	...
	var EMB_TRIPLE_OBJECT = '<{0,2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))>{4}';

	var TRIPLE_REF = TRIPLE_SUBJECT + '\\s' + TRIPLE_PRED + '\\s' + TRIPLE_OBJECT;
	var EMB_TRIPLE_REF = TRIPLE_SUBJECT + '\\s' + TRIPLE_PRED +
			'\\s(' + TRIPLE_SUBJECT + '|' + TRIPLE_OBJECT + ')\\s' +
			TRIPLE_PRED + '\\s(' + EMB_TRIPLE_OBJECT + '|' + TRIPLE_OBJECT  + '|' + TRIPLE_REF + ')';
	
	var tripleEls = [
		{ name: "TRIPLE_SUBJECT",
			regex:new RegExp("^"+TRIPLE_SUBJECT)},

		{ name: "TRIPLE_PRED",
			regex:new RegExp("^"+TRIPLE_PRED)},

		{ name: "TRIPLE_OBJECT",
			regex:new RegExp("^"+TRIPLE_OBJECT)},

		{ name: "EMB_TRIPLE_OBJECT",
			regex:new RegExp("^"+EMB_TRIPLE_OBJECT)}
	];
...
//		stringLiteralLongRegex.DOUBLE.quotes,
	...
	/**
	 *  Restores embedded triple from the array in, which we store elements
	 *  and checks if matches embedded triple patterns
	 * @returns {RegExpMatchArray}
	 */
	function validateTripleRef() {
		var tripleRef = '';
		for (var index = 0; index < processedTripleEls.length; index++) {
		...
		switch (getNumberOfStartingAngles()) {
			case 2 :
			case 3 :
				return tripleRef.match(new RegExp("^"+TRIPLE_REF), true, false);
			default : {
				return tripleRef.match(new RegExp("^"+EMB_TRIPLE_REF), true, false);
			}
		}
	}

	function getNumberOfStartingAngles() {
		if (processedTripleEls.length > 0) {
			var emb = processedTripleEls[0].match(new RegExp("<", "g"));
			if (emb) {
				return emb.length;
			}
		}
	...
	function tokenBase(stream, state) {
		...
		function nextToken() {
			...
			for (var index = 0; index < tripleEls.length; ++index) {
				...
				// This checks allows us to hack CodeMirror's
				// reading of tokens char by char
				if (evenTripleEl && index === 1) {
					continue;
				}
				// If already processed more than 3 elements from triple
				// check if there are four closing angles and if so skip second index
				if (processedTripleEls.length > 3 && stream.string.indexOf('>>>>') !== -1 && index === 2) {
					continue;
				}
				...
				if (consumed) {
					...
					for (var y = 0; y < terminals.length; ++y) {
						...
						if (consumed1) {
							...
							// without added angles to be able to suggest
							var autocompleteCall = isAutocompleteCall();
							var processed = {
								cat: autocompleteCall ? tripleEls[index].name : terminals[y].name,
								style: terminals[y].style,
								text: autocompleteCall ? consumed[0] : tripleVar.trim(),
								start: stream.start
							};
		...
		if (tokenOb.cat === "TRIPLE_SUBJECT"
			...
			|| tokenOb.cat === "TRIPLE_OBJECT" && isEmbTripleRef()) {
			return(tokenOb.style);
		}
		...
		var finished= false;
		var topSymbol;
		var token = processedTripleEls.length === 3 ? 'TRIPLE_REF' :
			(tokenOb.cat === "EMB_TRIPLE_OBJECT" ? 'EMB_TRIPLE_REF' : tokenOb.cat);
		...
		if (tokenOb.cat === "TRIPLE_OBJECT" || tokenOb.cat === 'EMB_TRIPLE_OBJECT') {
			processedTripleEls = [];
		}
