...
//	}
	...
	var TRIPLE_SUBJECT1 = '<{2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	var TRIPLE_SUBJECT2 = '<{4}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	var TRIPLE_SUBJECT3 = '<{6}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	var TRIPLE_SUBJECT4 = '<{8}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';

	var TRIPLE_PRED = '(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	var TRIPLE_OBJECT = '<{0,2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))(>{2}|>{4}|>{6}|>{8})';

	var TRIPLE_OBJECT1 = '<{0,2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))>{2}';
	var TRIPLE_OBJECT2 = '<{0,2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))>{4}';
	var TRIPLE_OBJECT3 = '<{0,2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))>{6}';
	var TRIPLE_OBJECT4 = '<{0,2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))>{8}';

	var TRIPLE_REF = TRIPLE_SUBJECT1 + '\\s' + TRIPLE_PRED + '\\s' + TRIPLE_OBJECT;

	var EMB_ON_SUBJECT_POS = '<<' + TRIPLE_REF + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_OBJECT;
	var EMB_ON_OBJECT_POS = TRIPLE_SUBJECT1 + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_REF + '>>';
	var EMB_ON_SUBJECT_OBJECT_POS = '<<' + TRIPLE_REF + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_REF + '>>';

	var DOUBLE_EMB_ON_SUBJECT_POS = EMB_ON_SUBJECT_POS + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_OBJECT;
	var DOUBLE_EMB_ON_OBJECT_POS = EMB_ON_OBJECT_POS + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_OBJECT;
	var DOUBLE_EMB_ON_BOTH_POS = EMB_ON_SUBJECT_OBJECT_POS + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_OBJECT;
	var DOUBLE_NESTED_SUBJ_SUBJ_FINAL_OBJECT = EMB_ON_SUBJECT_POS + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_REF;
	var DOUBLE_NESTED_SUBJ_OBJ_FINAL_OBJECT = '<<' + EMB_ON_OBJECT_POS + '\\s' + TRIPLE_PRED + '\\s'+ TRIPLE_REF;

	/**
	 *  Order in the array is really IMPORTANT
	 *  Try first to process first elements with
	 *  most opening and closing angles.
	 * @type {({regex: RegExp, name: string}|{regex: RegExp, name: string})[]}
	 */
	var tripleEls = [
		{ name: "TRIPLE_SUBJECT",
			regex:new RegExp("^"+TRIPLE_SUBJECT4)},

		{ name: "TRIPLE_SUBJECT",
			regex:new RegExp("^"+TRIPLE_SUBJECT3)},

		{ name: "TRIPLE_SUBJECT",
			regex:new RegExp("^"+TRIPLE_SUBJECT2)},

		{ name: "TRIPLE_SUBJECT",
			regex:new RegExp("^"+TRIPLE_SUBJECT1)},

		{ name: "TRIPLE_PRED",
			regex:new RegExp("^"+TRIPLE_PRED)},

		{ name: "TRIPLE_OBJECT",
			regex:new RegExp("^"+TRIPLE_OBJECT4)},

		{ name: "TRIPLE_OBJECT",
			regex:new RegExp("^"+TRIPLE_OBJECT3)},

		{ name: "TRIPLE_OBJECT",
			regex:new RegExp("^"+TRIPLE_OBJECT2)},

		{ name: "TRIPLE_OBJECT",
			regex:new RegExp("^"+TRIPLE_OBJECT1)}
	];

	/**
	 *  Patterns, which match different embedded triples.
	 *  Currently added most and could be easily extended
	 * @type {({regex: RegExp, name: string}|
	 * {regex: RegExp, name: string}|
	 * {regex: RegExp, name: string}|
	 * {regex: RegExp, name: string}|
	 * {regex: RegExp, name: string})[]}
	 */
	var embPatternsPossible = [

		{ name: "TRIPLE_REF",
			regex:new RegExp("^"+TRIPLE_REF)},

		{ name: "EMB_ON_SUBJECT_POS",
			regex:new RegExp("^"+EMB_ON_SUBJECT_POS)},

		{ name: "EMB_ON_OBJECT_POS",
			regex:new RegExp("^"+EMB_ON_OBJECT_POS)},

		{ name: "EMB_ON_SUBJECT_OBJECT_POS",
			regex:new RegExp("^"+EMB_ON_SUBJECT_OBJECT_POS)},

		{ name: "DOUBLE_EMB_ON_SUBJECT_POS",
			regex:new RegExp("^"+DOUBLE_EMB_ON_SUBJECT_POS)},

		{ name: "DOUBLE_EMB_ON_OBJECT_POS",
			regex:new RegExp("^"+DOUBLE_EMB_ON_OBJECT_POS)},

		{ name: "DOUBLE_EMB_ON_BOTH_POS",
			regex:new RegExp("^"+DOUBLE_EMB_ON_BOTH_POS)},

		{ name: "DOUBLE_NESTED_SUBJ_SUBJ_FINAL_OBJECT",
			regex:new RegExp("^"+DOUBLE_NESTED_SUBJ_SUBJ_FINAL_OBJECT)},

		{ name: "DOUBLE_NESTED_SUBJ_OBJ_FINAL_OBJECT",
			regex:new RegExp("^"+DOUBLE_NESTED_SUBJ_OBJ_FINAL_OBJECT)}
...
//		stringLiteralLongRegex.DOUBLE.quotes,
	...
	/**
	 *  Based on this check elements that are part of
	 *  embedded triple will be returned without
	 *  angles to be properly autocomplete
	 * @returns {boolean}
	 */
	function isAutocompleteCall() {
		var stackTrace = new Error().stack;
		return stackTrace.indexOf('isValidCompletionPosition') !== -1;
	}

	// Flag is raised when VALUES clause is detected
	var	valuesClauseDetected = false;
	// Stores elements part of TRIPLE_REF needed for check afterwards
	var processedTripleEls = [];

	/**
	 *  Checks if there are variables in embedded triple,
	 *  which is part of VALUES clause
	 * @param token
	 * @returns true if no variables present and false otherwise
	 */
	function tripleRefInVALUESClauseValid(token) {
		var valid = true;
		if (valuesClauseDetected) {
			if (token === 'TRIPLE_REF') {
				valid = tripleRefIsValid();
				valuesClauseDetected = false;
			}
		}
	...
	/**
	 *  True if embedded triple is valid and does not contain variables in it
	 * @returns {boolean}
	 */
	function tripleRefIsValid() {
		if (!validTripleRef()) {
			return false;
		}
		for (var index = 0; index < processedTripleEls.length; index++) {
			if (processedTripleEls[index].match(new RegExp("^" + VAR1), true, false)
				|| processedTripleEls[index].match(new RegExp("^" + VAR2), true, false)) {
				return false;
			}
		}
		return true;
	...
	/**
	 *  Restores embedded triple from processedTripleEls array,
	 *  and checks if matches embedded triple patterns
	 * @returns {boolean}
	 */
	function validTripleRef() {
		if (processedTripleEls.length === 0) {
			return false;
		}
		var tripleRef = '';
		for (var index = 0; index < processedTripleEls.length; index++) {
			if (index < processedTripleEls.length - 1) {
				tripleRef += processedTripleEls[index] + ' ';
			} else {
				tripleRef += processedTripleEls[index];
			}
		}

		for (var index = 0; index < embPatternsPossible.length; index++) {
			var matcher = tripleRef.match(embPatternsPossible[index].regex, true, false);
			if (matcher) {
				return true;
			}
		}
	...
	function tokenBase(stream, state) {
		...
		function nextToken() {
			...
			// Tokens part of TRIPLE_REF
			for (var index = 0; index < tripleEls.length; ++index) {
				var evenTripleEl = processedTripleEls.length % 2 === 0;
				// Only even elements are predicates in TRIPLE_REF
				// If stored already even element in the array
				// skip this check to avoid messing predicate and object
				if (evenTripleEl && index === 4) {
					continue;
				}
				// Don't try to consume predicates and objects
				// if subject part of triple hasn't been found
				if (processedTripleEls.length === 0 && index >= 4) {
					break;
				}
				consumed = stream.match(tripleEls[index].regex, true, false);
				if (consumed) {
					var tripleVar = consumed[0];
					if (evenTripleEl) {
						// Find number of opening and closing angles and remove them based on number
						var numbOf = tripleVar.match(new RegExp("<", "g"));
						if (numbOf) {
							if (numbOf.length % 2 === 0) {
								tripleVar = tripleVar.substring(numbOf.length);
							} else {
								tripleVar = tripleVar.substring(numbOf.length - 1);
							}
						} else {
							numbOf = tripleVar.match(new RegExp(">", "g"));
							if (numbOf) {
								if (numbOf.length % 2 === 0) {
									tripleVar = tripleVar.substring(0, tripleVar.length - numbOf.length);
								} else {
									tripleVar = tripleVar.substring(0, (tripleVar.length - numbOf.length - 1));
								}
							}
						}
					}

					// Extract style of the element part of TRIPLE_REF from terminal elements
					for (var y = 0; y < terminals.length; ++y) {
						var consumed1 = tripleVar.match(terminals[y].regex, true, false);
						if (consumed1) {
							// In case of autocomplete call return elements
							// without added angles to allow autocomplete suggest
							var autocompleteCall = isAutocompleteCall();
							var processed = {
								cat: autocompleteCall ? terminals[y].name : tripleEls[index].name,
								style: terminals[y].style,
								text: autocompleteCall ? tripleVar.trim() : consumed[0],
								start: stream.start
							};
							// Don't store elements if it is a autocomplete call
							if (!autocompleteCall) {
								processedTripleEls.push(consumed[0]);
							}
							return processed;
						}
					}
		...
		if (tokenOb.cat === "TRIPLE_SUBJECT"
			|| tokenOb.cat === "TRIPLE_PRED"
			|| tokenOb.cat === "TRIPLE_OBJECT" && !validTripleRef()) {
			return(tokenOb.style);
		}
		...
		var finished= false;
		var topSymbol;
		var token = tokenOb.cat === "TRIPLE_OBJECT" ? 'TRIPLE_REF' : tokenOb.cat;
		...
		// Incremental LL1 parse
			while(state.stack.length>0 && token && state.OK && !finished ) {
				...
				if (!ll1_table[topSymbol]) {
					...
					if (topSymbol == token && tripleRefInVALUESClauseValid(token)) {
						if (token === 'VALUES') {
							valuesClauseDetected = true;
						}
						// Matching terminals
