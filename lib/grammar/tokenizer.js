...
//	}
	...
	var TRIPLE_SUBJECT = '<{2}(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	var TRIPLE_PRED = '(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + '))';
	var TRIPLE_OBJECT = '(' + IRI_REF + '|' + PNAME_LN + '|(' + VAR1 + '|' + VAR2 + ')|' +
		'((' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + '|' + STRING_LITERAL_LONG + ')((\\^\\^' + PNAME_LN + ')|' + LANGTAG + ')?|' +
		INTEGER + '|'+ DECIMAL + '|'+ DOUBLE + '|'+
		INTEGER_POSITIVE + '|' + DECIMAL_POSITIVE + '|' + DOUBLE_POSITIVE + '|'+
		INTEGER_NEGATIVE + '|' + DECIMAL_NEGATIVE + '|' + DOUBLE_NEGATIVE + '))>>';

	var EMB_TRIPLE_REF = TRIPLE_SUBJECT + '\\s' + TRIPLE_PRED + '\\s' + TRIPLE_OBJECT;
	...
	var terminals= [
		...
		{ name: "EMB_TRIPLE_REF",
			regex:new RegExp("^"+EMB_TRIPLE_REF),
			style:"variable-3" },
	...
	function tokenBase(stream, state) {
		...
		function nextToken() {
			...
			for (var i=0; i<terminals.length; ++i) {
				...
				if (consumed) {
					// if (i === 3) {
					// 	var tripleVar = consumed[0].split(" ");
					// }
					return {
			...
			// // Tokens defined by embedded triple regular expressions
			// for (var i=0; i<tripleEls.length; ++i) {
			// 	consumed= stream.match(tripleEls[i].regex,true,false);
			// 	if (consumed) {
			// 		var tripleVar = tripleEls[i];
			// 		if (i === 0) {
			// 			tripleVar = tripleVar.substring(2);
			// 		} else if (i === 2) {
			// 			tripleVar = tripleVar.substring(0, tripleVar.length - 2);
			// 		}
			// 		for (var y = 0; y < terminals.length; ++y) {
			// 			var consumed1 = tripleVar.match(terminals[y].regex, true, false);
			// 			if (consumed1) {
			// 				console.log(consumed1);
			// 				return {
			// 					cat: tripleVar.name,
